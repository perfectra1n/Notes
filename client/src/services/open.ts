...
async function openCustom(type: OpenType, entityId: string, mime: string) {
    checkType(type);
...
async function openExternally(type: OpenType, entityId: string, mime: string) {
    checkType(type);
    ...
    if (utils.isElectron()) {
        ...
        const res: boolean = !!(await electron.shell.openPath(resp.tmpFilePath));
